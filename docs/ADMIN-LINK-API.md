...
The identity demo is complete after this hosted acceptance run. No additional module is necessary for that story. The metrics endpoint is currently a GraphQL-style stub: it does not parse a schema or honor field selection. Present it as a prototype, or implement real GraphQL before claiming a GraphQL integration. Auth0 user creation and authorization metadata are still manual; Postman simulates the provisioning client. The demo is not a production deployment. Export data before the free database expires.
